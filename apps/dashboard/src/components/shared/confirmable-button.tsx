...
const ConfirmableButton: React.FC<Props> = ({ title, disabled, onConfirm }) => {
  ...
  return disabled ? (
    <div className="rounded-full border border-secondary-dark-40 bg-secondary-dark-10 p-1">
      <button
        className="h-7 w-32 rounded-full bg-secondary-dark-10 p-1 text-center text-xs text-secondary-dark-50"
        disabled
      >
        {title}
      </button>
    ...
    <button
      className={cn('group rounded-full p-1', {
        'bg-secondary-dark-40': disabled,
        'bg-secondary-dark-20': loading,
        'cursor-pointer border-1.5 border-gmc-berry bg-zinc-800 hover:border-primary hover:bg-gmc-berry':
          !loading && !confirmable,
        'cursor-pointer border-1.5 bg-gmc-heart-light-40 hover:border-primary hover:bg-gmc-heart':
          confirmable,
      })}
      disabled={loading}
      onClick={handleClick}
    >
      <div
        className={cn(
          'flex h-7 w-32 items-center justify-center rounded-full text-xs',
          {
