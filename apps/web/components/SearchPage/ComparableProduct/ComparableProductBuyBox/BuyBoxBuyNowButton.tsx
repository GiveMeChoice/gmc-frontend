...
const BuyBoxBuyNowButton: React.FC<Props> = ({ product }) => {
  return (
    <div className="active:bg-transarent z-0 w-full rounded-full bg-zinc-900 hover:bg-zinc-900 hover:shadow-md active:shadow-sm">
      <a
        href={product.offerUrl}
        target="_blank"
        rel="noreferrer"
        className="z-20 flex w-full items-center justify-between gap-x-4
      rounded-full border border-zinc-800 bg-gradient-to-l from-gmc-surf-light-50 to-primary-light-50 py-2.5 px-8 pl-4 text-[16px] text-zinc-800 transition-transform duration-200 ease-in-out hover:-translate-y-[6px]  active:translate-y-0
      "
      >
